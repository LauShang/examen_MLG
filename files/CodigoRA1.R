...
#-Working directory-
wdir<-"C:/Users/javie/OneDrive - INSTITUTO TECNOLOGICO AUTONOMO DE MEXICO/MaestriaEnCienciaDeDatos/2doSemestre/RegresionAvanzada/CodigoRAr2"
setwd(wdir)
...
sig2<-4
n<-3
#Parámetros información inicial
th0<-39
sig20<-219.47
...
z2<-out.a[,2,1]
par(mfrow=c(3,2))
plot(z1,type="l",col="grey50", main="Traza de la cadena", xlab="iteración", ylab=" valor del parámetro")
lines(z2,col="firebrick2")
y1<-cumsum(z1)/(1:length(z1))
y2<-cumsum(z2)/(1:length(z2))
ymin<-min(y1,y2)
ymax<-max(y1,y2)
plot(y1,type="l",col="grey50",ylim=c(ymin,ymax), main="Promedios ergódicos", xlab="iteración", ylab="media")
lines(y2,col="firebrick2",ylim=c(ymin,ymax))
hist(z1,freq=FALSE,col="grey50", main = "Distribución posterior cadena 1", xlab="valor del parámetro", ylab="conteo")
hist(z2,freq=FALSE,col="firebrick2", main = "Distribución posterior cadena 2", xlab="valor del parámetro", ylab="conteo")
cadena1 <- acf(z1, plot=FALSE)
plot(cadena1, main= "Autocorrelación cadena 1")
cadena2 <- acf(z2, plot=FALSE)
plot(cadena2, main= "Autocorrelación cadena 2")
...
ej3.sim<-bugs(data,inits,parameters,model.file="Ej3.txt",
              n.iter=20000,n.chains=2,n.burnin=2000,n.thin=30)
...
#Predictions
#Seleccionamos los renglones de out.sum con yf
out.yf<-out.sum[grep("yf",rownames(out.sum)),]
or<-order(calif$MO)
#seleccionamos la escala de los ejes con respecto a los valores min y max
#entre los datos y las estimaciones del modelo
ymin<-min(calif$SP,out.yf[,c(1,3,7)])
ymax<-max(calif$SP,out.yf[,c(1,3,7)])
par(mfrow=c(1,1))
plot(calif$MO,calif$SP,ylim=c(ymin,ymax))
#Graficamos la predicciones ordendando los valores obtenidos con respecto a
...
n<-dim(precio)[1]
m<-precio$num #Numero de habitaciones reservadas
...
mod35.sim<-bugs(data,inits,parameters,model.file="Ej35.txt",
               n.iter=1000,n.chains=2,n.burnin=100,n.thin=1,debug=TRUE)
#JAGS
...
#JAGS
out.dic<-mod35.sim$BUGSoutput$DIC
#
print(out.dic)
...
#--- Ejemplo 4 ---
#TAREA
salarios <- read.table("https://gente.itam.mx/lnieto/index_archivos/salarios.txt", header=TRUE)

n<-nrow(salarios)
par(mfrow=c(1,1))
plot(salarios$X1, salarios$Y)
plot(salarios$X2, salarios$Y)
plot(salarios$X3, salarios$Y)

cor(salarios)
#Nuevos datos
xf= matrix(data = c(5.4,17,6,6.2,12,5.8,6.4,21,6.1), nrow = 3, ncol=3, byrow = TRUE)
x= data.matrix(salarios[,2:4], rownames.force = NA)

#Definimos los datos de entrada para el modelo
data<-list("n"=n,"y"=salarios$Y,"x"=x, "m"=3, "xf"=xf )

#Valores iniciales de la cadena
inits<-function(){list(alpha=1,beta=rep(0,3),tau=1,yf1=rep(0,n),yf2=rep(0,3))}

#Nodos a monitorear
parameters <- c("alpha","beta","tau", "yf1", "yf2")

#Ejecución del código
ej4.sim <- bugs(data, inits, parameters, model.file = "Ej4.txt", n.iter=10000, n.chains=2, n.burnin = 1000, n.thin=100)
traceplot(ej4.sim)

#Convergencia de las cadenas
#OpenBUGS
out.a<-ej4.sim$sims.array

#JAGS
out.a<-ej4.sim$BUGSoutput$sims.array
...
or<-order(salarios$X2)
#seleccionamos la escala de los ejes con respecto a los valores min y max
#entre los datos y las estimaciones del modelo
ymin<-min(salarios$Y,out.yf[,c(1,3,7)])
ymax<-max(salarios$Y,out.yf[,c(1,3,7)])
par(mfrow=c(1,1))
plot(salarios$X2,salarios$Y,ylim=c(ymin,ymax))
#Graficamos la predicciones ordendando los valores obtenidos con respecto a
#la variable explicativa
lines(salarios$X2[or],out.yf[or,1],lwd=2,col=2)
lines(salarios$X2[or],out.yf[or,3],lty=2,col=2)
lines(salarios$X2[or],out.yf[or,7],lty=2,col=2)
...
ej5a.sim<-bugs(data,inits,parsa,model.file="Ej5a.txt",
              n.iter=50000,n.chains=2,n.burnin=5000,n.thin=10)
...
n<-nrow(desastres)
plot(desastres,type="l", xlab="año")
plot(desastres[2:n,2]-desastres[1:(n-1),2],type="l", ylab="Y(t)-Y(t-1)", xlab= "Tiempo")
#Podemos observar un problema de heteroselasticidad
...
ej6a2.sim<-bugs(data,initsa2,parsa2,model.file="Ej6a2.txt",
                n.iter=20000,n.chains=2,n.burnin=1000,n.thin=5)
...
par(mfrow=c(1,1))
plot(desastres,type="l",col="grey80",ylim=c(ymin,ymax),xlab="Año")
...
#--- Ejemplo 7 ---
#-Reading data-
leucemia<-read.table("http://gente.itam.mx/lnieto/index_archivos/leucemia.txt",header=TRUE)
n<-nrow(leucemia)
par(mfrow=c(2,2))
plot(leucemia$Obs)
...
#OpenBUGS
out.a<-ej7c.sim$sims.array

#JAGS
out.a<-ej7a.sim$BUGSoutput$sims.array
...
out.sum.t<-cbind(out.sum.t,apply(out.a$theta,2,prob))
dimnames(out.sum.t)[[2]][4]<-"prob"
print(out.sum.t)
...
ymax<-5
par(mfrow=c(1,1))
plot(leucemia$Obs/leucemia$Pops*10000,type="p",col="grey50",xlim=c(xmin,xmax),ylim=c(ymin,ymax), ylab = "Tasa de mortalidad por cada 10,000 niños", xlab= "Categoría")
#
out.p<-outb.p
...
#
out.p<-outc.p
points((1:8),out.p[,5],col=4,pch=16,cex=0.5)
segments((1:8),out.p[,3],(1:8),out.p[,7],col=4)
#
...
library(tidyverse)
df <- data.frame(outc.sum)

#--- Ejemplo 8 ---
#-Reading data-
reclama<-read.table("http://gente.itam.mx/lnieto/index_archivos/reclama.txt",header=TRUE)
n<-nrow(reclama)
par(mfrow=c(2,2))
plot(reclama$r)
...
#OpenBUGS
out.a<-ej8c.sim$sims.array

#JAGS
out.a<-ej8.sim$BUGSoutput$sims.array

z1<-out.a[,1,1]
z2<-out.a[,2,1]
par(mfrow=c(3,2))
plot(z1,type="l",col="grey50", main="Traza de la cadena", xlab="iteración", ylab="eta")
lines(z2,col="firebrick2")
y1<-cumsum(z1)/(1:length(z1))
y2<-cumsum(z2)/(1:length(z2))
ymin<-min(y1,y2)
ymax<-max(y1,y2)
plot(y1,type="l",col="grey50",ylim=c(ymin,ymax), main="Promedios ergódicos", xlab="iteración", ylab="eta")
lines(y2,col="firebrick2",ylim=c(ymin,ymax))
hist(z1,freq=FALSE,col="grey50", main = "Distribución posterior cadena 1", xlab="eta", ylab="conteo")
hist(z2,freq=FALSE,col="firebrick2", main = "Distribución posterior cadena 2", xlab="eta", ylab="conteo")
cadena1 <- acf(z1, plot=FALSE)
plot(cadena1, main= "Autocorrelación cadena 1")
cadena2 <- acf(z2, plot=FALSE)
plot(cadena2, main= "Autocorrelación cadena 2")
...
ymax<-1
par(mfrow=c(1,1))
plot(reclama$r/reclama$n,type="p",col="grey50",xlim=c(xmin,xmax),ylim=c(ymin,ymax), xlab = "Tipo de seguro", ylab="Probabilidad de reclamo")
#
out.p<-outb.p
points(out.p[,1],col=2,pch=16,cex=0.5)
...
#--- Ejemplo 9 ---
#-Reading data-
milk<-read.table("http://gente.itam.mx/lnieto/index_archivos/milk.txt",header=TRUE)
milk$t<-1970:1982
n<-nrow(milk)
plot(milk$x,milk$y,type="n",xlab = "Millones de vacas", ylab = "Producción de leche [lbsx10^9]")
text(milk$x,milk$y,labels=milk$t,cex=0.5,col=1)
plot(milk$t,milk$y,type="l", xlab="Año", ylab="Producción de leche [lbsx10^9]")
plot(milk$t,milk$x,type="l", xlab="Año", ylab="Millones de vacas")
...
initsa2<-function(){list(beta=rep(0,3),tau=1,yf1=rep(1,n))}
initsb1<-function(){list(beta=rep(0,n+m),tau.y=1,tau.b=1, yf1=rep(0,n+m), g=1)}
initsb2<-function(){list(beta=rep(0,n+m),tau.y=1,yf1=rep(0,n+m), g=1)}
initsc<-function(){list(beta=rep(0,n),tau.y=1,tau.b=1,yf1=rep(0,n))}
...
#-Running code-
#OpenBUGS
ej9a.sim<-bugs(data,initsa2,parsa,model.file="Ej9a.txt",
               n.iter=50000,n.chains=2,n.burnin=5000, n.thin=50)
ej9b.sim<-bugs(data,initsb2,parsb2,model.file="Ej9b.txt",
               n.iter=50000,n.chains=2,n.burnin=5000, n.thin=50)
ej9c.sim<-bugs(data,initsc,parsc,model.file="Ej9c.txt",
               n.iter=50000,n.chains=2,n.burnin=5000, n.thin=50)
ej9d.sim<-bugs(data,initsd,parsd,model.file="Ej9d.txt",
               n.iter=10000,n.chains=2,n.burnin=1000, n.thin=40)
#JAGS
ej9a.sim<-jags(data,initsa1,parsa,model.file="Ej9a.txt",
              n.iter=50000,n.chains=2,n.burnin=5000,n.thin=1)
ej9b.sim<-jags(data,initsb2,parsb1,model.file="Ej9b.txt",
               n.iter=50000,n.chains=2,n.burnin=5000,n.thin=40)
...
#OpenBUGS
out.a<-ej9.sim$sims.array

#JAGS
out.a<-ej9.sim$BUGSoutput$sims.array
...
#t vs y
par(mfrow=c(1,1))
plot(data$t,data$y,type="b",col="grey80",ylim=c(ymin,ymax),xlim=c(xmin,xmax), xlab="Años")
lines(data$t,out.yf[1:n,1],col=2)
lines(data$t,out.yf[1:n,3],col=2,lty=2)
lines(data$t,out.yf[1:n,7],col=2,lty=2)
lines(data$t[n]:(data$t[n]),out.yf[n,1],col=4)
lines(data$t[n]:(data$t[n]),out.yf[n,3],col=4,lty=2)
lines(data$t[n]:(data$t[n]),out.yf[n,7],col=4,lty=2)
...
#--- Ejemplo 10 ---
#-Reading data-
mercado<-read.table("http://gente.itam.mx/lnieto/index_archivos/mercado.txt",header=TRUE)
mercado.ts<-ts(mercado,start=c(1990,1),end=c(1991,52),frequency=52) #El método ts convierte los datos a una serie de tiempo
n<-nrow(mercado)
mercado$Tiempo<-1:n
plot(mercado.ts, main="")
...
data<-list("n"=n,"y"=scale(mercado$SHARE)[1:n],"x1"=mercado$PRICE,"x2"=mercado$OPROM,"x3"=mercado$CPROM) #scale resta media y divide entre la desviación estándar
...
initsd<-function(){list(beta=rep(0,n),tau.b=1,yf1=rep(1,n))}
...
ej10b.sim<-bugs(data,initsb,parameters,model.file="Ej10b.txt",
               n.iter=10000,n.chains=2,n.burnin=1000, n.thin = 100)
...
x<-data$x1
par(mfrow=c(1,1))
plot(x,y,type="p",col="grey50",ylim=c(ymin,ymax), xlab = "PRICE_t", ylab = "S_t")
...
segments(x,out.yf[,3],x,out.yf[,7],col=2)
par(mfrow=c(1,1))
plot(x,y,type="l",col="grey50",ylim=c(ymin,ymax), xlab= "Semana", ylab= "S_t")
...
out.beta<-out.sum[grep("beta",rownames(out.sum)),]
par(mfrow=c(3,1))
plot(out.beta[1:104,1],type="l", xlab="t", ylab="beta_1t")
plot(out.beta[105:208,1],type="l", xlab="t", ylab="beta_2t")
plot(out.beta[209:312,1],type="l", xlab="t", ylab="beta_3t")
...
scallop %>% ggplot(aes(x = tot.catch))+
  geom_histogram(bins=30)+
  xlab("Número de ostines encontrados")+
  ylab("Número de sitios")
scallop$lgcatch<-log(scallop$tot.catch+1)
hist(scallop$lgcatch)

maps::map("usa",xlim=c(-74,-71),ylim=c(38.2,41.5))
...
plot(scallop.var, xlab = "distancia", ylab = "variograma")

#variograma te?rico
g1<-function(x,s2,phi){
  out<-s2*(1-exp(-phi*x))
  out
}
g2<-function(x,s2,phi){
  out<-s2*(1-exp(-(phi^2)*(x^2)))
  out
}
x<-seq(0,1,,100)
s2<-1
...
y2<-g2(x,s2,phi)
ymax<-max(y1,y2)
plot(x,y1,ylim=c(0,ymax),type="l", xlab= "distancia", ylab= "variograma")
...
ej11.sim<-bugs(data,inits,parameters,model.file="Ej11.txt",
               n.iter=2000,n.chains=2,n.burnin=1000,n.thin=3)
#JAGS
...
#OpenBUGS
out.a<-ej11.sim$sims.array

#JAGS
out.a<-ej11.sim$BUGSoutput$sims.array
...
#OpenBUGS
out.a<-ej12.sim$sims.array

#JAGS
out.a<-ej12.sim$BUGSoutput$sims.array
