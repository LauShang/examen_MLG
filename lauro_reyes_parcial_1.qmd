...
# muestras
theta_samples <- fit$draws(variables = "theta")
...
ggplot(theta_df, aes(x = theta)) +
  ...
  labs(title = "Distribución Posterior con Distribución a priori Beta(90,60)",
...
ggplot(theta_df, aes(x = theta)) +
  geom_histogram(aes(y = ..density..), bins = 20, fill = "skyblue", color = "black") +
  geom_vline(xintercept = theta_mean, color = "red", linetype = "dashed", size = 1) +
  labs(title = "Distribución Posterior con Distribución a priori Normal(0.6,0.0016)",
       x = "Tasa de Créditos Otorgados (theta)") +
  theme_minimal()
```
```{r}
